...
import { CoSoItem } from '../types';
import { 
  Building, Plus, Search, Filter, RefreshCw,
  CheckCircle2, AlertCircle, Sparkles, Download, Phone, MapPin, 
  Key, Layers, X, Check, Edit2
} from 'lucide-react';

export const CoSoTable: React.FC = () => {
  ...
  const handlePushRow = async (item: CoSoItem) => {
    ...
    const result = await pushRecordToAppSheet(item);
    ...
    if (success) {
      ...
      setSyncLogs(prev => [`[AppSheet Error] ${result.error || 'Không rõ lỗi'} — "${item.ten_co_so}" (${item.id})`, ...prev]);
    }
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (pushToAppSheetImmediately) {
      ...
      newItem.trang_thai_dong_bo = result.success ? 'Đã đổ data Co_so' : 'Lỗi đồng bộ';
      if (!result.success) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Mã hóa tự động <span className="font-bold text-slate-800 font-mono">id</span> (CS001, CS002...), lưu trữ <span className="font-bold text-slate-800">Ten_co_so</span> và <span className="font-bold text-slate-800">dia_chi</span>
            </p>
          </div>
        </div>
